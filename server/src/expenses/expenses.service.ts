...
export class ExpensesService {
    ...
    async list(params: {
        ...
        const pageSize = Math.max(
            1,
            Math.min(200, Number(params.pageSize) || 20)
        )
        ...
        if (params.year) {
            ...
            const to = params.month
                ? new Date(params.year, params.month, 0, 23, 59, 59, 999)
                : new Date(params.year + 1, 0, 0, 23, 59, 59, 999)
    ...
    async importRows(dto: ImportExpensesDto) {
        ...
        const currency =
            dto.defaultCurrency && dto.defaultCurrency in Currency
                ? (dto.defaultCurrency as Currency)
                : Currency.RWF
        for (let idx = 0; idx < rows.length; idx++) {
            const row = rows[idx] as Record<string, unknown>
            try {
                const title = (row.Details ||
                    row.details ||
                    row.title ||
                    '') as string
                const dateStr = (row.Date || row.date || '') as string
                const catName =
                    ((row.Category || row.category) as string) ||
                    'Uncategorized'
                const rawAmount = (row['Amount (RWF)'] ||
                    row.amount ||
                ...
                await this.prisma.expense.create({
                    data: {
                        ...
                        notes: (row.Notes || row.notes || undefined) as
                            | string
                            | undefined,
                    },
                })
    ...
    async summary(year?: number) {
        ...
        if (year) {
            where.expenseDate = {
                ...
                lte: new Date(year + 1, 0, 0, 23, 59, 59, 999),
            }
        }
    ...
    async monthly(year?: number) {
        if (year) {
            ...
            >`
        SELECT EXTRACT(MONTH FROM "expenseDate") as month, SUM(amount) as total
        FROM "Expense"
